...
export class AngularEditorService {
  ...
  createLink(url: string) {
    if (!url.includes('http')) {
      ...
      const classList =
        (this.savedSelection?.commonAncestorContainer?.parentElement as HTMLAnchorElement)?.classList.value || '';
      const newUrl = '<a href="' + url + '" target="_blank" class="' + classList + '">' + this.selectedText + '</a>';
  ...
  createCustomClass(customClass: CustomClass) {
    ...
    if (customClass) {
      ...
      if (customClass.saveHref) {
        const href = (this.savedSelection?.commonAncestorContainer?.parentNode as HTMLAnchorElement)?.href || '';
        newTag =
          '<' +
          tagName +
          ' class="' +
